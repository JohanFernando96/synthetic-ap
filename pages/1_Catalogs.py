...
def main() -> None:
    ...
    with vendors_tab:
        st.dataframe(_as_df([v.model_dump() for v in cat.vendors]), use_container_width=True)

    with items_tab:
        st.dataframe(_as_df([i.model_dump() for i in cat.items]), use_container_width=True)
    ...
    with mapping_tab:
        ...
        st.dataframe(_as_df(vi), use_container_width=True)
